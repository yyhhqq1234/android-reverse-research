...
- 工作区：`NECR/work_necr/`，所有产出只写这里：
  ...
  - `release_v1/PATCHES.md` — 去壳 v1 补丁清单 P1–P4（必读）
  - `tools/` — apktool 3.0.3、Il2CppDumper 6.7.46、AssetStudioMod、BlackDex32、frida-server、GG 脚本、`assemble_unshelled.py`、`patch_iap.py`、`g1_gate.py` 等
  - `src/` — apktool 反编译树（manifest 已去 StubApp）
  - `build/` / `repack/` / `prod/` / `trial/` / `logs/` / `deps/` / `save/` — 构建、成品、内存 dump、对照试验、日志、依赖、存档
- 关键结论（不要重复踩坑）：
...
6. **密钥与敏感信息**：本地测试签名用的 keystore / jks 与口令仅保存在本机，不上传、不改口令、不提交到外部仓库（公开仓库已通过 `.gitignore` 排除）。
