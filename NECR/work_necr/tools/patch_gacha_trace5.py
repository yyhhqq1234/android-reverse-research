#!/usr/bin/env python3
"""V17F: bracket the two calls inside 0x999D88 (test, cumulative on v17b/c/d/e).

v17e proved crash is after A04 (0x998B40 -> 0x999D88), before A05.
Inside 0x999D88 the only live calls: vanilla bl 0x12896E0 (B01) and the
historical idx0 GOD gate bl 0x1B3B928 (B02, pass-through while idx0=0).
PRE/POST bracketing decides which subtree owns the fault.
Cave @0x1AABE78+760 (FARM2). Test build only.
"""
import struct
import sys
...
HOOKS = [
    ('B01', 0x999E20, None, 'bl', 0x12896E0),
    ('B02', 0x999E74, None, 'bl', 0x1B3B928),
]
...
for s in strs:
    ...
    offs[s] = cur
    cur += (len(s) + 3) // 4 * 4
assert cur <= 0x1AABE78 + 1636, 'cave overflow'
